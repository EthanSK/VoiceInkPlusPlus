...
6. Treat Ethan's normal app use as an ongoing source of runtime evidence. At the start of a regression investigation and immediately after each reported reproduction, verify the installed build/PID, run `scripts/live-delivery-trace.sh status`, and inspect `show` before changing code again. Correlate the report time with the physical button route, captured destination, paste, focus boundary, auto-send action, latency, and verifier. Logs prove only what VoiceInk++ attempted; require surface state or Ethan's observation before claiming the destination handled it.
7. State any relevant prior constraint before making a risky change.
...
- **Primary button** is the preferred name for Ethan's normal recording control. **Normal button**, **thumb button**, **toggle button**, **recording button**, **same button**, and historical **G5** are aliases. First press starts; the same primary button again performs a normal stop (`focusedAtStop`). If the start app remained continuously keyboard-focused, that route uses base VoiceInk's live caret; after any app activation it uses only the exact stop-time input. It never uses `recordingStart`.
...
| Action | Destination |
| --- | --- |
| Primary button again while recording | Normal stop (`focusedAtStop`): base VoiceInk live caret while the start app has remained continuously focused; after any activation, only the exact stop-time input; never fall back to `recordingStart` |
...
Insulate ordinary primary dictation from latch experiments. Snapshot a monotonic external-app activation generation and keyboard-focused app PID at recording start. Only `focusedAtStop` may bypass exact AX identity when that continuity remains unbroken at selection, Cmd-V, and Return; it then uses plain current-caret Cmd-V plus one ordinary HID Return with no AX read-back, restoration, semantic Send, background preparation, or retry. An activation-generation change—including away-and-back—must reject the fast path and preserve the exact stop-time route. `recordingStart` and `focusedDuringTranscription` must never enter it.
...
For Telegram compatibility work, read [references/telegram-live-test.md](references/telegram-live-test.md) before touching its UI or interpreting a trace. During any active destination/delivery reliability investigation—not only Telegram—use `scripts/live-delivery-trace.sh start|status|show|stop` instead of rebuilding an ad-hoc unified-log pipeline. This is temporary debug mode: its launchd-owned runner keeps only allowlisted destination/delivery metadata—including bounded semantic-Send readiness and foreground paste cancellation/setup failures—in private daily files under `~/Library/Logs/VoiceInkPlusPlus/DeliveryTrace`, prunes them after seven complete days, and never records dictated/transcribed or app content. A plain background or `nohup` child is reaped when Codex's bounded command shell exits and can silently leave the physical test untraced. Keep the trace running while Ethan reproduces issues; once the reliability investigation is genuinely closed, run `stop`, confirm `debugMode=off`, and say so in the handoff.
