...
For Telegram compatibility work, read [references/telegram-live-test.md](references/telegram-live-test.md) before touching its UI or interpreting a trace. Use `scripts/live-delivery-trace.sh start|status|show|stop` instead of rebuilding an ad-hoc unified-log pipeline; the script keeps only allowlisted destination/delivery metadata and manages its child log stream so repeated tests do not leave orphans.
