...
Do not treat Codex Computer Use's visible agent cursor as proof that macOS offers a second independent mouse or keyboard-focus channel. The installed helper's `VirtualCursor` is presentation and targeting state; its native bundle separately exposes refetchable AX trees, PID/window-addressed synthesized input, synthetic app-focus enforcement, system focus-steal prevention, and CPS focus-state notifications, while its Window2 API says input methods activate their target automatically. That broad immediate-action stack is useful architectural evidence, not a reusable API or runtime proof for VoiceInk++. Preserve VoiceInk++'s narrower delayed exact-input contract and live-test its own target: Computer Use forbids `com.openai.codex`, so never bypass that policy with custom live probes.
