...
For live compatibility work or an intermittent rapid-recording report, use `scripts/live-delivery-trace.sh start|status|show|gesture|stop` instead of rebuilding an ad-hoc unified-log pipeline. It writes daily mode-0600 traces under `~/Library/Logs/VoiceInkPlusPlus/DeliveryTrace`, removes files older than seven days, and keeps only allowlisted recording/job/audio lineage, provider timing/counts, transcript length/digest, destination/delivery metadata, exact-input identity state, and foreground paste/Return outcomes. Transcript text, prompts, provider error bodies, screenshots, and chat titles must never enter the trace; transcription failures retain lineage but redact the error body. Read [references/telegram-live-test.md](references/telegram-live-test.md) before touching Telegram's UI or interpreting its identity trace. The detached runner must remain launchd-owned: a plain background or `nohup` child is reaped when Codex's bounded command shell exits and can silently leave the physical test untraced.

For a failed or harsh-feeling physical Primary double/triple gesture, keep that managed trace running
and use `scripts/live-delivery-trace.sh gesture [line-count]`. Require one `primary gesture:` line per
press with monotonic timestamp/delta, both timing thresholds, phase transition, observed/classified
press number, generation/reset reason, capture eligibility, and reducer decision, followed by the
recovery-draft and clipboard-only completion boundaries when click three succeeds. Never reproduce
the gesture by moving/clicking Ethan's mouse. A click three beyond the logged continuation window is
a fresh gesture; do not widen it past the macOS multi-click interval merely to hide that evidence,
because a later independent double-click must never inherit the prior double.
