...
| Action | Destination |
| --- | --- |
| Primary button again while recording | Base VoiceInk normal stop: no saved input; system keyboard focus and current Mode at delivery (`primaryCurrentInput`) |
...
Primary permanently follows base VoiceInk's current Mode and keyboard-focused input at
delivery. It owns no saved destination. The two Next routes separately use exact delivery
whenever their capture/state requirements pass. Current source ignores the historical
`VIPPExactInputDeliveryEnabled` preference; do not treat it as a switch, route, supported
fallback, or reason to bypass Next capture. Keep the recorder's second destination slot
visible as a warning whenever no exact Next target is owned; returning a stale target
would lie about ownership, while hiding the slot would conceal that the Next route is
unavailable.
...
delivery crossed its cutoff, or no exact Next target is available. Eligible states still
