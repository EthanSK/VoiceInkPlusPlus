...
- Semantic Send is restricted to proven chat bundles and the nearest shared composer container, and requires an explicit Send label. Never press an unlabelled OpenAI square: the same slot can become an enabled Stop control while an agent runs, so retained wrapper/geometry is not semantic proof.
