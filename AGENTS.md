...
Signed v2.0.244 from commit `4fa8ec9` is the currently installed Telegram experiment (CDHash `98859c850d863e194281355fc4384a867f795d98`) with exact delivery enabled. Its fresh Mini bundle passed all 29 named unit tests and its installed signature/Automation entitlement verify, but it is not an accepted Telegram release until the disposable Saved Messages trace physically proves Primary, both Next routes, no focus theft, Send clear/reset, and safe wrong-chat rejection where a second disposable chat is available. A build/test/install result must not displace v2.0.243 as the rollback checkpoint.
