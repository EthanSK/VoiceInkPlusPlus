...
When no recording is active but a result is still transcribing, that same double-click also selects clipboard-only **Won’t paste** for the newest result identified at click one. Freeze this policy after the provider returns or fails, before Mode effects. Never substitute an older result, undo an issued paste, cancel the provider, or send another playback command. Keep single-click new recording, quiet provider-failure feedback, and the per-session recovery/HUD behavior unchanged.
