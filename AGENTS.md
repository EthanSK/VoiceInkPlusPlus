...
When no recording is active but a result is still transcribing, that same double-click also selects clipboard-only **Won’t paste** for the newest result identified at click one. Keep the bounded policy freeze open while clicks three and four can still continue that exact pending-session gesture: click three is an intermediate no-op, and click four restores normal delivery while suppressing that session's configured auto-send once. Never substitute an older result, undo an issued paste, cancel the provider, or send another playback command. With no eligible pending session, do not invent an idle quadruple action; keep one-click Start and two-click cancel unchanged. Keep single-click new recording, quiet provider-failure feedback, and the per-session recovery/HUD behavior unchanged.
