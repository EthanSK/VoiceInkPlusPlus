...
**Current runtime state verified 2026-07-20:** signed VoiceInk++ v2.0.244 is installed from commit
`4fa8ec9` with CDHash `98859c850d863e194281355fc4384a867f795d98` and exact delivery enabled.
Its fresh Mini bundle named and passed all 29 unit tests; deep/strict signing and the outer Automation
entitlement verify. This is an installed Telegram experiment, not accepted live compatibility: the
Saved Messages scenarios below still require physical trace and visible-result agreement.
...
| Destination | Saved input | Preferred non-activating insertion | Auto-send chain | Verification | Audited live evidence through installed v2.0.244 |
| --- | --- | --- | --- | --- | --- |
| Codex desktop | Exact Codex task composer | Targeted Unicode after verified internal window/editor focus | Explicit nearby semantic Send only; ordinary HID Return only while the exact composer owns system keyboard focus, with at most one retry after a readable unchanged composer | Exact composer clears/resets; rendered-message echo is optional telemetry; one issued action followed by an unreadable replacement remains indeterminate and requires matching user-visible confirmation | v243 commit `5475ef2` is the Primary-working checkpoint, not accepted latch support. A live `focusedDuringTranscription` attempt verified background insertion and issued one FooterActions action without focus theft, but Ethan confirmed the latch outcome did not submit. Both Next routes remain unresolved |
...
| Telegram | Exact Telegram message `AXTextArea` plus readable AX chat anchors, or an audited app/version/build/layout with a stable SHA-256 digest of the selected-chat header crop | `AXSelectedText`; targeted Unicode only after a proven pre-mutation AX no-op and readable AX anchors | Explicit nearby or exact retained labelled Send; ordinary HID Return only if the exact composer already owns system keyboard focus; no retry | Composer clears/resets; exact structure/internal focus plus matching anchors or a freshly matching visual digest; missing Screen Recording permission, blank/protected capture, tuple/layout drift, or any identity mismatch fails closed | v243 captured the exact foreground composer, but both live `focusedDuringTranscription` and `recordingStart` background runs failed before insertion because saved-window resolution returned no match. v244's isolated, privacy-bounded Telegram 12.9/282526 visual-identity candidate is installed and unit-tested but remains live-unverified until both latch routes, no-focus-theft, Send clear/reset, wrong-chat rejection, and the Primary regression check are physically proved |
