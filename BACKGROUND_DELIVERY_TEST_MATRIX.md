...
**Current runtime state verified 2026-07-21:** signed VoiceInk++ v2.0.245 is installed from commit
`99a596b` with CDHash `9857ba882d2ff2f9064edcf79f055dd9e7dccdd1` and exact delivery enabled.
Its fresh Mini bundle named and passed all 30 unit tests through direct `xcrun xctest` after the
canonical Xcode action built and stalled in TestManager; deep/strict signing, outer Automation, and
Screen Recording permission verify. Ethan and the trace accepted both Telegram background Next
routes in Saved Messages. Foreground Primary and wrong-chat fail-closed remain physically untested.
...
| Telegram | Exact Telegram message `AXTextArea` plus readable AX chat anchors, or an audited app/version/build/layout with a stable SHA-256 digest of the selected-chat header crop | One-shot `AXSelectedText`; bounded targeted Unicode only after full visual/AX identity revalidation when selected-text insertion is unavailable | On pinned Telegram 12.9/282526 only: fresh exact-chat revalidation, then one `telegramTargetedHIDReturn` sequence (HID source, modifier boundary, Return down/up, live modifier restoration); no retry or generic fallback | Composer clears/resets; exact structure/internal focus plus matching anchors or a freshly matching visual digest; missing Screen Recording permission, blank/protected capture, tuple/layout drift, or any identity mismatch fails closed | v245 physically passed both `recordingStart` and `focusedDuringTranscription` in Saved Messages while Terminal stayed frontmost: `visualIdentity=true`, exact background insertion verified, `route=telegramTargetedHIDReturn`, and `verification=verifiedCleared` in 66 ms and 40 ms. Foreground Primary and wrong-chat rejection remain not tested |
