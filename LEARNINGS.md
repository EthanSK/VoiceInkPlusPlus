...
**Commit:** see the commit whose subject begins `Fix second-chance transcription retarget auto-send`
**Guard:** The canonical second-chance scenario is: stop normally → transcription begins → focus a new editable input → press Next Track once → optionally move to another app → finished text pastes and uses the newly selected input's configured auto-send. This is not a toggle and not the recording-start route. `pendingPasteTargetCanChangeUntilDeliveryResolvesIt` now asserts that `.enter` travels with the retargeted target. Future agents must read this entry and `RECORDING_DESTINATIONS.md` before changing Next Track behavior.
---
