...
enum BoundedAppleScriptError: Error, LocalizedError {
    ...
    /// `osascript` and Terminal own stderr. Either can echo a script literal
    /// containing Ethan's transcript or native session identity, and this error is
    /// later shown and logged. Accept the bytes only to make their deliberate
    /// disposal testable; never retain or interpolate them.
...
/// Runs exact Terminal automation away from MainActor and kills the helper at a
/// hard deadline. The AppleScript source travels over stdin—not argv or the
/// environment—because it contains the transcript. This keeps a wedged Terminal
/// Apple Event from freezing the recorder UI or acting on a stale session later.
