...
enum RecorderPanelShortcutPolicy {
    static func shortcuts(
        explicitCancelShortcut: Shortcut?,
        canUseModeShortcuts: Bool
    ) -> [ShortcutAction: Shortcut] {
        var shortcuts: [ShortcutAction: Shortcut] = [:]

        // Bare Escape always belongs to the foreground app. It must never be registered or
        // swallowed by VoiceInk++ merely because a recorder panel is visible. Cancellation stays
        // available through the red X and an explicitly configured non-Escape shortcut.
        if let explicitCancelShortcut, !explicitCancelShortcut.isUnmodifiedEscape {
            shortcuts[.cancelRecorder] = explicitCancelShortcut
        }

        if canUseModeShortcuts {
            for (index, keyCode) in RecorderPanelShortcutManager.digitKeyCodes.enumerated() {
...
final class RecorderPanelShortcutManager: ObservableObject {
    ...
    private func refreshVisibleShortcuts() {
        ...
        let shortcuts = RecorderPanelShortcutPolicy.shortcuts(
            explicitCancelShortcut: ShortcutStore.shortcut(for: .cancelRecorder),
            canUseModeShortcuts: canUseModeShortcuts
        )
    ...
    fileprivate static let digitKeyCodes: [UInt16] = [
        UInt16(kVK_ANSI_1),
        UInt16(kVK_ANSI_2),
        UInt16(kVK_ANSI_3),
        UInt16(kVK_ANSI_4),
