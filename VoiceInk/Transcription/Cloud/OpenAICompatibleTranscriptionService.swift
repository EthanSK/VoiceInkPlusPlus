...
class OpenAICompatibleTranscriptionService {
    static let noSpeechDetectedErrorCode = "voiceink_no_speech_detected"
    ...
    /// The local Deepgram adapter uses one exact, non-secret code for a canonical
    /// successful upstream response that contains no speech. Recognize only that code
    /// and status; every other custom-provider body remains untrusted and redacted.
    /// This keeps silence out of the transient-502 retry path without allowing a
    /// provider to surface echoed prompts, URLs, credentials, or vocabulary.
    static func providerError(
        ...
        if statusCode == 422,
           let envelope = try? JSONDecoder().decode(
                TypedProviderErrorEnvelope.self,
                from: responseBody
           ),
           envelope.error.code == noSpeechDetectedErrorCode {
            return .noSpeechDetected
        }
        return .apiRequestFailed(
            statusCode: statusCode,
            message: sanitizedProviderErrorMessage(responseBody: responseBody)
        )
    }

    private struct TypedProviderErrorEnvelope: Decodable {
